...
function renderGoals(){
  ...
  grid.addEventListener('click', handleGoalCardClick);
}
...
function goalCardHtml(goal){
  ...
  <div class="card hoverable" data-id="${goal.id}">
    ...
    <div class="row-actions" style="margin-top:14px; justify-content:flex-end;">
      ${!goal.completed ? `<button data-act="complete" title="Mark complete">✓ Complete</button>` : ''}
...
function handleGoalCardClick(e){
  const btn = e.target.closest('button[data-act]');
  if(!btn) return;
  const card = e.target.closest('[data-id]');
  const id = card.dataset.id;
  const goal = state.goals.find(g => g.id === id);
  if(btn.dataset.act === 'edit') openGoalModal(goal);
  if(btn.dataset.act === 'delete') deleteGoal(id);
  if(btn.dataset.act === 'complete') completeGoal(goal);
}
...
function handleRowClick(e){
  ...
  if(btn.dataset.act === 'edit') openTaskModal(task);
  if(btn.dataset.act === 'delete') deleteTask(id);
}

function toggleTask(task){
  task.completed = !task.completed;
  saveState();
  if(task.completed){ addXp(10); showToast('Task completed (+10 XP)'); }
...
function handleHabitClick(e){
  ...
  if(btn.dataset.act === 'toggle'){
    const t = todayStr();
    if(habit.log[t]) delete habit.log[t];
    else { habit.log[t] = true; addXp(5); showToast('Habit logged (+5 XP)'); }
...
function openHabitModal(){
  ...
  `, (modal) => {
    ...
    $('#habitForm', modal).addEventListener('submit', (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      state.habits.push({ id: uid(), title: fd.get('title'), category: fd.get('category'), log:{} });
...
function noteCardHtml(n){
  return `<div class="card note-card" data-id="${n.id}">
    ...
    <div class="row-actions" style="justify-content:flex-end;"><button data-act="delete">🗑 Delete</button></div>
  </div>`;
}
